...
def plot_images(images,labels,data_name):
    ...
    for n in range(1, n_samples+1):
        ...
        if(n_channels<3):
            plt.imshow(images[n-1],"gray")
        else:
            plt.imshow(images[n-1])
        plt.title(f"{np.argmax(labels[n-1])}")
        plt.xticks([])
        plt.yticks([])
...
def format_mnist(plot=False):
    ...
    if(plot):
        ...
        plot_images(X_train[selection],y_train[selection],data_name)
